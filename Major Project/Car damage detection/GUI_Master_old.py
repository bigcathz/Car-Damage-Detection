...
#
lbl = tk.Label(root, text="Car Damage Detection", font=('times', 35,' bold '), width=65,height=1,bg="brown",fg="white")
lbl.place(x=0, y=0)
...
def test_model_proc(fn):
    ...
    if fn!="":
        ...
        model = load_model('vehicle_model1.h5',compile=False)
        ...
        if yoga == 0:
            Cd="Damage Car Detected upto 10-30% "
        
        if yoga == 1:
            Cd="Damage is not detected"
        
        if yoga == 2:
            Cd="Damage is detected upto 40-60%"
            
        if yoga == 3:
            Cd="Damage is detected upto 70-90%"
      
            
        A=Cd
        return A
    img = open(fn)    
    myfile = open("E:/Harddisk/Mayuri Groups/Mayuri Groups/23SS201- Car Damage Detection/Car damage detection 100% code/testing")
    if img == myfile:
          print(myfile)
...
def openimage():
    ...
    global fn
    print(fn)
    fileName = askopenfilename(initialdir='E:/Harddisk/Mayuri Groups/Mayuri Groups/23SS201- Car Damage Detection/Car damage detection 100% code/testing', title='Select image for Aanalysis ',
                               filetypes=[("all files", "*.*")])
    IMAGE_SIZE=200
    imgpath = fileName
    fn = fileName
